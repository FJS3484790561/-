...
输出质量要求（必须执行）：
11. 最终提示词不是一句概括，也不是风格关键词列表；目标长度为 900–1800 个中文字符，必须写成一段连续、可直接复制给图像模型的具体指令。不要为了简短而省略空间结构、布局、材料、灯光、尺度或负面约束。
12. 最终提示词按这个顺序完整覆盖：原图与相机锁定 → 必须保留和禁止改变 → 布局、收纳与动线 → 墙地顶材料 → 家具、织物、绿植和装饰 → 自然光与人工灯光 → 真实施工尺度、重量、接触、遮挡和收口 → 摄影质感 → 明确负面约束。
13. 对原图无法确认的结构，写“保持原样”，不得臆造拆墙、改门窗、改层高或新增不可解释的结构。对用户没有要求的区域做克制处理，不要堆满物品。
14. 设计风格只作为文字方向。不要提及、引用或使用控制台上传的风格参考图片；该图片不会传给你，也不属于本提示词输入。
15. 不要输出分析过程、分点标题、解释、致歉、Markdown 代码围栏或第二个版本；只输出一个“【最终图生图提示词】”标记和其后一段完整提示词。
请按以下格式输出：
【最终图生图提示词】
