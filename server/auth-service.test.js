...
function createFixture() {
  ...
  const service = new AuthService({
    ...
    clock: () => now,
    mailer: { sendPasswordReset: async (message) => sent.push(message) },
  })
...
test('registers a user and rejects duplicate email', async () => {
  ...
  assert.equal((await service.register({ email: 'User@Example.com', password: 'correct-horse' })).ok, true)
...
test('login creates a session that survives lookup and logout', async () => {
  const { service } = createFixture()
  await service.register({ email: 'user@example.com', password: 'correct-horse' })
