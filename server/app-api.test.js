...
test('lets administrators issue limited redemption codes and users redeem them once', async () => {
  ...
  assert.equal(list.data.redemptionCodes[0].code, created.data.code)
...
test('accepts isolated feedback and sends an idempotent reward code', async () => {
  const messages = []
  const runtime = createAppRuntime({ mailer: { sendFeedbackReward: async (message) => messages.push(message) } })
  await runtime.provisionAdmin({ password: 'correct-horse' })
  const adminCookie = await login(runtime.api, 'admin@example.com')
  const userCookie = await registerAndLogin(runtime.api, 'feedback@example.com')
  const submitted = await call(runtime.api, '/api/feedback', { method: 'POST', cookie: userCookie, body: { message: '希望增加更多风格。' } })
  assert.equal(submitted.response.status, 201)
  const listed = await call(runtime.api, '/api/admin/feedback', { cookie: adminCookie })
  assert.equal(listed.data.feedback[0].email, 'feedback@example.com')
  const accepted = await call(runtime.api, `/api/admin/feedback/${listed.data.feedback[0].id}`, { method: 'POST', cookie: adminCookie, body: { decision: 'accept', credits: 10 } })
  assert.equal(accepted.response.status, 200)
  assert.equal(accepted.data.feedback.status, 'accepted')
  assert.equal(accepted.data.feedback.rewardCredits, 10)
  assert.equal(messages.length, 1)
  assert.equal(messages[0].email, 'feedback@example.com')
  const repeated = await call(runtime.api, `/api/admin/feedback/${listed.data.feedback[0].id}`, { method: 'POST', cookie: adminCookie, body: { decision: 'accept', credits: 99 } })
  assert.equal(repeated.response.status, 200)
  assert.equal(messages.length, 1)
  assert.equal((await call(runtime.api, '/api/admin/feedback', { cookie: userCookie })).response.status, 403)
})
